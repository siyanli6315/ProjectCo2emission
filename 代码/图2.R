...
tot_region=read.csv("tot_region.csv",header=T,sep=",")
pc_region=read.csv("pc_region.csv",header=T,sep=",")
pg_region=read.csv("pg_region.csv",header=T,sep=",")

totx=melt(tot_region)
pcx=melt(pc_region)
pgx=melt(pg_region)
all_region=data.frame(fill=rep(c("二氧化碳排放总量","每GDP二氧化碳排放量","人均二氧化碳排放量"),each=152),region=as.character(totx$Country.name),year=rep(1990:2008,each=8),y=c(totx$value,pgx$value,pcx$value))
write.csv(all_region,"all_region.csv",row.names=F)
